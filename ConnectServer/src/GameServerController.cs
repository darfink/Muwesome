...
namespace Muwesome.ConnectServer {
  internal class GameServerController : IGameServerController {
    ...
    private readonly ConcurrentDictionary<ushort, GameServer> gameServers;

    /// <summary>Initializes a new instance of the <see cref="GameServerController"/> class.</summary>
    public GameServerController() =>
      this.gameServers = new ConcurrentDictionary<ushort, GameServer>();
    ...
    /// <inheritdoc />
    public IReadOnlyCollection<GameServer> Servers => this.gameServers.Values.AsReadOnly();
    ...
    public GameServer GetServerByCode(ushort code) {
      if (this.gameServers.TryGetValue(code, out GameServer server)) {
        return server;
      }
